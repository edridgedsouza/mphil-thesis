...
type <- 'normal' # 'normal' or 'starvation
options <- c(normal='GSM4132287_Normal_finalaggr', starvation='GSM4132288_Starvation_finalaggr')
exp_matrix <- Read10X(paste0('data/', options[type]))
saveRDS(object = exp_matrix@Dimnames[[1]] %>% unique(), file = 'fullGeneList.RDS')
s_o <- CreateSeuratObject(counts = exp_matrix, project=type)
...
# Find dimensionality of dataset with heatmap inspection, JackStraw resampling, or elbow plot
  # DimHeatmap(s_o, dims = 1:10, cells = 1000, balanced = TRUE)
...
Cairo::CairoPDF('figs/umap.pdf', width=5, height=5)
...
FeaturePlot(s_o, features = c("SoxN", "D"))
...
Cairo::CairoPDF('figs/RidgePlot.pdf', height=5, width=10)
RidgePlot(s_o, features = c("SoxN", "D"))
dev.off()
